...
1. [Functionalities](#functionalities)
   ...
   1. [EnvironmentHelper](#environmenthelper)
...
1. [AppConfigurator](#appconfigurator)
1. [AppNotReadyError](#appnotreadyerror)
1. [ConfigData](#configdata)
...
1. [MainWindowPosition](#mainwindowposition)
...
This is intended as a super class of all specific error classes used during development. It encapsulates a message (required) and a JS error object (or anything throw during execution) (default null). It also provides a string classifier field (default null), in the case it would be necessary to provide additional typing info.
...
The `_className` is a protected field that must be overridden in any child class. This variable must be initialized with the child class name. This is not obtained during runtime because the minification can change the class name.

The `_classifier` is another protected field created to act as as classifier string. It's value can be set in constructor but a more common use is to override this field in subclasses.

### InvalidPlatformError

This is an [ErrorWrapper](#errorWrapper) generated by a try to run the app in a operational system different from win32, linux or darwin (macOs), if the app runs at all.

An object of this class has a classifierf with the value `PLATFORM_ERROR` and the message `The operational system ${process.platform} is not supported.`.

To use:

```typescript
import { InvalidPlatformError } from '@alandrade21/electron-arch';
```
...
An object of this class has a classifier with the value `RUNTIME_ERROR`.

To use:

```typescript
import { InvalidParameterError } from '@alandrade21/electron-arch';
```
...
```typescript
import { UnexpectedError } from '@alandrade21/electron-arch';
```

### AppNotReadyError

This is an [ErrorWrapper](#errorWrapper) to represent a state where the `app.on('ready')` event was not fired yet.

An object of this class has a classifier with the value `RUNTIME_ERROR`, and a message with the value `The app.on('ready') event was not fired yet.`.

To use:

```typescript
import { AppNotReadyError } from '@alandrade21/electron-arch';
```
...
An object of this class has a classifier with the value `FS_ERROR`, but this classifier is commonly overwritten with the error code generated by the file system.

To use:

```typescript
import { ConfigFileError } from '@alandrade21/electron-arch';
```
...
An object of this class has a classifier with the value `FS_ERROR`, but this value is commonly overwritten with the error code generated by the file system.

To use:

```typescript
import { DatabaseFileError } from '@alandrade21/electron-arch';
```
...
An object of this class has a classifier with the value `INIT_ERROR`, and a message with the value `Main window already initialized`.

To use:

```typescript
import { MainWindowAlreadyInitializedError } from '@alandrade21/electron-arch';
```
...
An object of this class has a classifier with the value `INIT_ERROR`, and a message with the value `Main window not initialized`.

To use:

```typescript
import { MainWindowNotInitializedError } from '@alandrade21/electron-arch';
```

### I18nError

This is an [ErrorWrapper](#errorWrapper) generated by this library i18n engine.

An object of this class has a classifier with the value `I18N_ERROR`.

To use:

```typescript
import { I18nError } from '@alandrade21/electron-arch';
```

## EnvironmentHelper

Class to help the developer to verify various aspects of the electron environment.

To use:

```typescript
import { envHelper } from '@alandrade21/electron-arch';
```

To see some environment variables, use `envHelper.printEnvironment()`. This method prints its output to the console.

## MainWindowController

Class to encapsulate the main window initialization process and to grant static access to the main window variable across the app.
...
After `app.on('ready')` event call the static method `MainWindowController.initialize(pos)`.

If called before `app.on('ready')` event, a [AppNotReadyError](#appnotreadyerror) will be thrown.

If the main window is already initialized when this method is called, an  [MainWindowAlreadyInitializedError](#mainwindowalreadyinitializederror) will be thrown.

The `pos` parameter is an [MainWindowPosition](#mainwindowposition) instance.
...
If the app is fired in development environment (`app.isPackaged` return false) with the --serve parameter, the main window will be loaded with the content of `http://localhost:4200`. Else, the main window will be loaded with the file `./dist/index.html`.

If the app is fired in development environment (`app.isPackaged` return false), the dev tools will be opened.

### Main window access

To access the main window use static call `MainWindowController.mainWindow`. This will throw an [MainWindowNotInitializedError](#mainwindownotinitializederror) if the main window were not initialized yet.

## MainWindowPosition

Class to represent the position and size of the electron main window. An instance of this class is expected by the [MainWindowController initialization method](#main-window-initialization).

To use:

```typescript
import { MainWindowPosition } from '@alandrade21/electron-arch';
```

A MainWindowPosition instance has four properties:

* x: x coordinate for the window top left corner.
* y: y coordinate for the window top left corner.
* width: window width in pixels.
* heigh: window heigh in pixels.

You can create create an instance passing this values in the constructor.

There also is an static method `MainWindowPosition.getMaximizedInstance()` that returns an instance to create a maximized window in the primary display.

## ConfigData

When your electron app needs its configuration options saved inside a configuration file, this file is represented by an object that holds all configuration data. This object is an instance of a class that implements the ConfigData interface.
...
If there is an permission access error, a [ConfigFileError](#configfileerror) is thrown with classifier `EPERM`. If an unknown error occurs, a [ConfigFileError](#configfileerror) is thrown with that information and a classifier from file system, if it exists. In both cases, the original error is stored.
...
If there are errors, a [ConfigFileError](#configfileerror) is thrown. If the error was caused by permission access, the error has the `EPERM` classifier. If the file does not exists, the error has the classifier `ENOENT`. If the error is unknown and there is a file system error type, it will be put in the classifier field. If there is not possible to generate the object of the type `T` from the raw data read, the classifier will be `PARSE_ERROR`. In all cases, the original error object is preserved.

### writeFile(data: T): void

Writes the config file to the disk, replacing its contents if it exists. If the directory does not exists, creates it. The parameter `data` is a object of the concrete type representing the configuration options (T - see [instantiation section](#configfilemanager-instantiation) above). This is a synchronous service.

If the parameter `data` is not informed, throws a [InvalidParameterError](#invalidparametererror).

If the file system reports a problem, a [ConfigFileError](#configfileerror) is thrown, preserving the original error type in the classifier field.
...
The next two parameters are intended to allow a "production" version to run in the same machine used to development, preserving the production database and configuration files.
...
* If the environment is development (`app.isPackaged` return false), the config folder is set to the value informed in the `devConfigFolderPath` parameter and the data folder is set to the value informed in the `devDataFolderPath` parameter.
...
Method to start the app configuration. This super class execute the existence check of the config file. If it do exist, the file is read and the options object is initialized. If it does not exist, the method that creates the config file is called.
